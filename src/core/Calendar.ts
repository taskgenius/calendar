...
export class Calendar<T = Dayjs> {
  ...
  private render(): void {
    // Check if we can do an incremental update (same view type, existing structure)
    const existingCalendar = this.container.querySelector(".tg-calendar");
    const existingViewBody =
      existingCalendar?.querySelector(".tg-view-container");
    const isSameView = this.activeView?.getMeta().type === this.currentViewType;
    ...
    // Incremental update: only update view body if structure exists and view type unchanged
    if (existingCalendar && existingViewBody && isSameView && this.activeView) {
      // Update context
      this.viewContext.currentDate = this.currentDate;

      // Update header title (in-place)
      const titleEl = existingCalendar.querySelector(".tg-title");
      if (titleEl) {
        titleEl.textContent = this.getHeaderTitle();
      }

      // Update view switcher active state
      const viewBtns = existingCalendar.querySelectorAll(".tg-view-btn");
      viewBtns.forEach((btn) => {
        const btnEl = btn as HTMLElement;
        if (btnEl.dataset["viewType"] === this.currentViewType) {
          btnEl.classList.add("tg-active");
        } else {
          btnEl.classList.remove("tg-active");
        }
      });

      // Clear only the view body, not the entire container
      clearElement(existingViewBody as HTMLElement);

      // Re-render view content into existing container
      this.activeView.render(
        existingViewBody as HTMLElement,
    ...
    // Full rebuild: view type changed or first render
    clearElement(this.container);
  ...
  private mergeConfig(config: CalendarConfig): ResolvedCalendarConfig {
    ...
    if (config.onRenderEvent) resolved.onRenderEvent = config.onRenderEvent;

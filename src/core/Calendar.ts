...
export class Calendar<T = Dayjs> {
	...
	constructor(
		...
		if (typeof containerSelector === "string") {
			...
			if (!el) {
				throw new Error(
					`Calendar container not found: ${containerSelector}`,
				);
			}
		...
		this.adapter = (config.dateAdapter ||
			new DayJsAdapter()) as DateAdapter<T>;
	...
	private mergeConfig(config: CalendarConfig): ResolvedCalendarConfig {
		...
		const dateFormats = {
			...
			dateTime:
				config.dateFormats?.dateTime || DEFAULT_DATE_FORMATS.dateTime,
		...
		if (config.onRenderMoreEventsPopover)
			resolved.onRenderMoreEventsPopover =
				config.onRenderMoreEventsPopover;
